  ...
  const script = function(p5) {
    ...
    const drawParticles = function() {
      ...
      for (let i = 0; i < particles.length; i++) {
        const posi = particles[i][0];
        for (let j = i + 1; j < particles.length; j++) {
          const posj = particles[j][0];
          const dist = posi.dist(posj);
      ...
      for (let i = 0; i < particles.length; i++) {
        const pos = particles[i][0];
        const speed = particles[i][1];
        const randSize = p5.random(3, 7);
        p5.ellipse(pos.x, pos.y, randSize, randSize);
        pos.add(speed);

        const distToMouse = mousePos.dist(pos);

        if (distToMouse < repelDist) {
          const repel = p5.createVector(pos.x - mousePos.x, pos.y - mousePos.y);
          const distFrac = (repelDist - distToMouse) / repelDist;
